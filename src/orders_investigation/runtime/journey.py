...
def orders_data_boundary(*, region: str = "eu-west-1") -> DataBoundary:
...
def orders_conformance_receipt(
    *,
    ...
    passed_checks: frozenset[str] | None = None,
) -> ConformanceReceipt:
    return ConformanceReceipt(
        ...
        else frozenset({"trace", "policy", "rollback", "authority", "placement"}),
        frozenset(),
    )
...
def run_orders_launch(
    *,
    ...
    receipt: ConformanceReceipt | None = None,
    ...
    receipt = receipt or orders_conformance_receipt()
    ...
    missing_rollback_only = conformance_reasons == (
        "required_check_missing:rollback",
    )
    if conformance_reasons and not missing_rollback_only:
    ...
    caller_allowed, _ = authorize_caller(
        orders_caller(),
        delegation or orders_delegation(),
        "orders-investigator",
        "report.write",
        NOW,
    )
    boundary = boundary or orders_data_boundary()
    try:
        target_id = place(boundary, orders_execution_targets())
    except ValueError:
        target_id = None
    ...
    evidence = LaunchEvidence(
        ...
        data_boundary_proven=target_id == "orders-eu-west-1",
    )
    ...
    execution = (
        run_placed_orders_investigation(boundary, orders_execution_targets())
        if decision[0]
        else None
    )
