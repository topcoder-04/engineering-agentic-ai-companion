"""Run Chapter 17's responsibility boundary from its real package."""

import importlib


def main() -> None:
    boundary = importlib.import_module("orders_investigation.coordination.delegation")
    public = sorted(name for name in vars(boundary) if not name.startswith("_"))
    print("CHAPTER 17 BOUNDARY")
