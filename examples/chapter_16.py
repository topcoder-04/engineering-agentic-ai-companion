"""Run Chapter 16's responsibility boundary from its real package."""

import importlib


def main() -> None:
    boundary = importlib.import_module("orders_investigation.graph.planning")
    public = sorted(name for name in vars(boundary) if not name.startswith("_"))
    print("CHAPTER 16 BOUNDARY")
