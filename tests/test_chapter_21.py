...
def test_orders_investigation_cannot_write_report_after_evidence_turns_stale():
    ...
    assert accepted.report_version == 8
    assert [step.kind for step in accepted.steps] == [
        "observe", "decide", "observe", "effect"
    ]
    assert refused.completed is False
    assert refused.refusal == "current_policy_denied:evidence_current_mismatch"
    assert refused.report_version == 7
    assert refused.report_content == "Investigating recovery."
