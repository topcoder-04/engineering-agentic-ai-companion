...
def test_demo_separates_execution_from_artifact_admission():
    completed = subprocess.run(
        [sys.executable, "examples/chapter_14.py", "--plain"],
        check=True,
        capture_output=True,
        text=True,
    )
    ...
    assert "Execution sandbox and artifact admission" in output
    assert "✓ ARTIFACT ADMITTED" in output
    assert "network_denied · execution" in output
    assert "output_too_large · artifact admission" in output
    assert "invalid_artifact · artifact admission · artifact_shape_mismatch" in output
